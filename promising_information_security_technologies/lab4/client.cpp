...
int main() {
    ...
    // -------------------------- MQV
    LoadParams("zikl-podg.txt", p, q, g, 0);
    mpz_t longterm_open_key, longterm_open_key_Y, longterm_secret_key;
    mpz_inits(longterm_open_key, longterm_open_key_Y, longterm_secret_key, 0);

    MQVEncrypt(p, q, g, longterm_secret_key, longterm_open_key);
    MQVEncrypt(p, q, g, secret, open);
    ...
    tmp = mpz_get_str(NULL, 0, longterm_open_key);
    str = tmp;
    str += '\0';
    send(socketClient, str.c_str(), str.length(), 0);
    ...
    recv(socketClient, ttmp, 2048, 0);
    str = ttmp;
    mpz_set_str(longterm_open_key_Y, str.c_str(), 0);

    MQVDecrypt(p, q, secret, open, open_Y, longterm_secret_key,
               longterm_open_key_Y, Z);
